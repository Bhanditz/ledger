...
describe('TransactionRegularStrategy Cashin transactions', () => {
  const accountService = new AccountService();
  const walletService = new WalletService();
  const transactionService = new TransactionService();
  const providerService = new ProviderService();

  let account1, account2, walletUsdAccount1, walletCreditCardAccount1, accountProvider, provider, walletProvider;
  ...
  beforeEach(async () => {
    ...
    walletCreditCardAccount1 = await walletService.insert({
      OwnerAccountId: account1.id,
      currency: 'USD',
  ...
  describe('Receiver Paying Fees(default behaviour)', () => {
    ...
    it('account1 cashes in from its creditcard wallet to its USD wallet(Without Platform Payment Provider Fees) Should generate 4 transactions(2 Regarding Wallet Provider fees and 2 regarding the transaction itself )', async () => {
      const amountTransaction = 1500;
      const currencyTransaction = 'USD';
      const cashinResult = await transactionService.insert({
        FromWalletId: walletCreditCardAccount1.id,
        ToWalletId: walletUsdAccount1.id,
      ...
      // account1 should be ToAccountId on Debit and FromAccountId on Credit transactions
      expect(providerDebitTransaction.ToAccountId).to.be.equal(account1.id);
      expect(providerCreditTransaction.FromAccountId).to.be.equal(account1.id);
      ...
      // account1 should have its original FromWalletId(walletCreditCard.id) present in the ToWalletId on Debit and FromWalletId on Credit transactions
      expect(providerDebitTransaction.ToWalletId).to.be.equal(walletCreditCardAccount1.id);
      expect(providerCreditTransaction.FromWalletId).to.be.equal(walletCreditCardAccount1.id);
      // Checking Amounts
    ...
    }); /** End of "account1 cashes in from its creditcard wallet to its USD wallet(Without Platform Payment Provider Fees) Should generate 4 transactions(2 Regarding Wallet Provider fees and 2 regarding the transaction itself )" */

    it('account1 cashes in from One wallet to Another wallet With Platform Fees and No Payment provider Fee should generate 6 transactions(2 wallet provider, 2 platform fees and 2 regular transaction)', async () => {
      ...
      // account1 should be ToAccountId on Debit and FromAccountId on Credit transactions
      expect(platformDebitTransaction.ToAccountId).to.be.equal(account1.id);
      expect(platformCreditTransaction.FromAccountId).to.be.equal(account1.id);
      // amount should be negative in Debit transaction and the same in Credit transaction
      ...
      // account1 should have its original FromWalletId(walletCreditCard.id) present in the ToWalletId on Debit and FromWalletId on Credit transactions
      expect(platformDebitTransaction.ToWalletId).to.be.equal(walletCreditCardAccount1.id);
      expect(platformCreditTransaction.FromWalletId).to.be.equal(walletCreditCardAccount1.id);
      ...
      // account1 should be ToAccountId on Debit and FromAccountId on Credit transactions
      expect(walletProviderDebitTransaction.ToAccountId).to.be.equal(account1.id);
      expect(walletProviderCreditTransaction.FromAccountId).to.be.equal(account1.id);
      // checking if amount of transactions is as expected
      const expectedFee = provider.fixedFee + (provider.percentFee * amountTransaction);
      // amount should be negative in Debit transaction and the same in Credit transaction
      ...
      // account1 should have its original FromWalletId(walletCreditCard.id) present in the ToWalletId on Debit and FromWalletId on Credit transactions
      expect(walletProviderDebitTransaction.ToWalletId).to.be.equal(walletCreditCardAccount1.id);
      expect(walletProviderCreditTransaction.FromWalletId).to.be.equal(walletCreditCardAccount1.id);
    ...
    }); /** End of "account1 cashes in from One wallet to Another wallet With Platform Fees and No Payment provider Fee should generate 6 transactions(2 wallet provider, 2 platform fees and 2 regular transaction)" */

    it('account1 cashes in from One wallet to Another wallet With Payment provider and No Platform Fee should generate 6 transactions(2 wallet provider, 2 payment provider fees and 2 regular transaction)', async () => {
      ...
      // account1 should be ToAccountId on Debit and FromAccountId on Credit transactions
      expect(paymentProviderDebitTransaction.ToAccountId).to.be.equal(account1.id);
      expect(paymentProviderCreditTransaction.FromAccountId).to.be.equal(account1.id);
      // amount should be negative in Debit transaction and the same in Credit transaction
      ...
      // account1 should have its original FromWalletId(walletCreditCard.id) present in the ToWalletId on Debit and FromWalletId on Credit transactions
      expect(paymentProviderDebitTransaction.ToWalletId).to.be.equal(walletCreditCardAccount1.id);
      expect(paymentProviderCreditTransaction.FromWalletId).to.be.equal(walletCreditCardAccount1.id);
      // Validating Provider Fees Transactions
      // Provider Account should be FromAccountId on Debit and ToAccountId on Credit transactions
      expect(walletProviderDebitTransaction.FromAccountId).to.be.equal(accountProvider.id);
      expect(walletProviderCreditTransaction.ToAccountId).to.be.equal(accountProvider.id);
    ...
    }); /** End of "account1 cashes in from One wallet to Another wallet With Payment provider and No Platform Fee should generate 6 transactions(2 wallet provider, 2 payment provider fees and 2 regular transaction)" */

    it('account1 cashes in from One wallet to Another wallet With Payment provider and No Platform Fee should generate 8 transactions(2 wallet provider, 2 platform fees, 2 payment provider fees and 2 regular transaction)', async () => {
      // Creates PaymentProvider Account
      const paymentProviderAccount = await accountService.insert({ slug: 'payment-provider' });
      // Creating USD Wallet of PaymentProvider
      const paymentProviderWallet = await walletService.insert({
    ...
    }); /** End of "account1 cashes in from One wallet to Another wallet With Payment provider and No Platform Fee should generate 8 transactions(2 wallet provider, 2 platform fees, 2 payment provider fees and 2 regular transaction)" */

    it('account1 cashes in from One wallet to Another wallet When the Wallet Provider has 0 fee so should generate only 2 transactions', async () => {
    ...
    }); /** End of "account1 cashes in from One wallet to Another wallet When the Wallet Provider has 0 fee so should generate only 2 transactions" */

    it('account1 cashes in from its USD Wallet to its Credit Card wallet Should pay no fees because the money is already in its USD Account(Should only pay Wallet provider Fee if wallet has no balance)', async () => {
      ...
      const walletUsdBalance = await walletLib.getCurrencyBalanceFromWalletId(currencyTransaction, walletUsdAccount1.id);
      ...
      cashinResult = await transactionService.insert({
        ...
        ToWalletId: walletCreditCardAccount1.id,
        amount: amountTransaction,
        currency: currencyTransaction,
      });
      // check if initial Cashin transaction generates 4 transactions(normal and wallet provider fee transactions, DEBIT AND CREDIT)
      ...
      expect(normalCreditTransaction.transactionGroupTotalAmount).to.be.equal(normalCreditTransaction.amount);
      expect(normalDebitTransaction.transactionGroupTotalAmount).to.be.equal(-1 * normalDebitTransaction.amount);
    }); /** End of "account1 cashes in from its USD Wallet to its Credit Card wallet Should pay no fees because the money is already in its USD Account(Should only pay Wallet provider Fee if wallet has no balance)" */
  });
...
}); /** End of "TransactionRegularStrategy Cashin transactions" */
